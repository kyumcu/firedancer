...
dump_cmd_args( int      * argc,
  ...
  char const * link     = fd_env_strip_cmdline_cstr( argc, argv, "--link",     NULL, "all"       );
...
dump_cmd_fn( args_t *         args,
  ...
  for( ulong i=0UL; i<config->topo.link_cnt; i++ ) {
    if( !strcmp( args->dump.link_name, config->topo.links[ i ].name ) || !strcmp( args->dump.link_name, "all" ) ) {
